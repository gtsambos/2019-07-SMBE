...
\tikzset{greynode/.style={font=\footnotesize,node distance=4cm and 2cm,fill=black!10,draw=black!30,inner sep=0pt,minimum size=8mm,shape=circle},
mutations/.style={shape=starburst,fill=red!50!blue,inner sep=0.6pt,starburst points=11,starburst point height=.5cm}}
...
% Haplotypes
\node[font=\footnotesize] (hap1) at ($(topLeft) + (0,5)$) {\footnotesize$\textrm{Sample 0}$};
\node[font=\footnotesize] (hap2) at ($(topLeft) + (0,4)$) {\footnotesize$\textrm{Sample 1}$};
\node[font=\footnotesize] (hap3) at ($(topLeft) + (0,3)$) {\footnotesize$\textrm{Sample 2}$};
